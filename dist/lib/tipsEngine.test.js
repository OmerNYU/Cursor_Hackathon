...
describe('TipsEngine', () => {
    ...
    describe('feature and subscore evaluation', () => {
        it('evaluates based on features', () => {
            const rules = [
                {
                    ...
                    windowSec: 0.5,
                    cooldownSec: 5,
                    when: (f) => f.gazeDevDeg > 10,
                    message: 'Look at camera',
                },
            ];
            const engine = new TipsEngine(rules);
            const badGaze = { ...mockFeatures, gazeDevDeg: 15 };
            // Establish activation at t=0ms
            ...
            expect(tips).toHaveLength(1);
            engine.reset();
            const goodGaze = { ...mockFeatures, gazeDevDeg: 3 };
        ...
        it('evaluates based on subscores', () => {
            ...
            const engine = new TipsEngine(rules);
            const badPosture = { ...mockSubscores, P: 0.5 };
            // Establish activation at t=0ms
            ...
            expect(tips).toHaveLength(1);
            engine.reset();
            const goodPosture = { ...mockSubscores, P: 0.9 };
            // Establish at t=0ms (condition false)
...
//# sourceMappingURL=tipsEngine.test.js.map