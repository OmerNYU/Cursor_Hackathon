...
import { WEIGHTS, POSTURE_GOOD_MAX_DEG, SWAY_GOOD_MAX_NORM, GAZE_GOOD_MAX_DEG, GAZE_MAX_BREAKS_PER_MIN, WRIST_VEL_STD_GOOD_MAX, WRIST_VEL_STD_HIGH, TORSO_SPEED_HIGH, } from './config.js';
/**
...
export function computeSubscores(features) {
    ...
    const S = 1 - clamp((wristVelStd - WRIST_VEL_STD_GOOD_MAX) /
        (WRIST_VEL_STD_HIGH - WRIST_VEL_STD_GOOD_MAX), 0, 1);
...
export function computeOverall(subscores) {
    const weighted = WEIGHTS.P * subscores.P +
 ...
 */
export function computeScores(features) {
